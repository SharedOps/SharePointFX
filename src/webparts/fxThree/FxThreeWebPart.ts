...
import * as strings from 'FxThreeWebPartStrings';
import FxThreeViewModel, { IFxThreeBindingContext } from './FxThreeViewModel';

let _instance: number = 0;

export interface IFxThreeWebPartProps {
  description: string;
}

export default class FxThreeWebPart extends BaseClientSideWebPart<IFxThreeWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: IFxThreeBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        viewModel: FxThreeViewModel,
        template: require('./FxThree.template.html'),
        synchronous: false
      }
    );
  }
